...
function Events() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-violet-50 via-purple-50 to-sky-50">
        <div className="text-center">
          <div className="inline-block animate-spin rounded-full h-14 w-14 border-b-4 border-violet-400 mb-4"></div>
  ...
  if (events.length === 0) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-gradient-to-br from-violet-50 via-purple-50 to-sky-50">
        <div className="text-center max-w-md bg-white rounded-2xl shadow-lg p-10 border border-violet-100">
          <div className="w-20 h-20 rounded-lg flex items-center justify-center mx-auto mb-6 bg-gradient-to-br from-violet-300 to-purple-400 shadow-md">
          ...
          <Link
            to="/create-event"
            className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-violet-500 to-purple-500 text-white font-semibold rounded-lg shadow-sm hover:shadow transition transform hover:-translate-y-0.5"
          >
  ...
  return (
    <div className="min-h-screen pt-28 py-12 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-violet-50 via-purple-50 to-sky-50">
      <div className="max-w-7xl mx-auto">
        <div className="mb-10">
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-violet-100 relative">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div>
                <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-1">
              ...
              <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                  ...
                  >
                    {sortOptions.map((opt) => (
                      <li
                        ...
                        className={`px-4 py-2 text-sm cursor-pointer text-gray-700 hover:bg-violet-50 transition ${filterStatus === opt.value ? "bg-violet-50 font-semibold" : ""}`}
                      >
                        {opt.label}
                      </li>
                    ))}
                ...
                <Link
                  to="/create-event"
                  className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-gradient-to-r from-violet-500 to-purple-500 text-white font-semibold rounded-lg shadow-sm hover:shadow transform hover:-translate-y-0.5 text-sm"
                >
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                  </svg>
        ...
        {filteredEvents.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-lg shadow-sm border border-violet-100">
            <svg className="mx-auto mb-3 w-10 h-10 text-violet-300" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
          ...
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {filteredEvents.map((event) => {
              ...
              const remaining = remainingSlots > 0 ? `${remainingSlots} slots` : "Full";

              return (
                <article
                  key={event._id}
                  className="group bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 transform hover:-translate-y-1 border border-violet-100"
                >
                  <div className="relative h-48 overflow-hidden">
                    {event.image || event.imageData ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-violet-200 via-purple-200 to-sky-200">
                    ...
                    <div className="absolute top-3 right-3">
                      <span
                        className={`px-3 py-1 rounded-md text-xs font-semibold backdrop-blur-sm border ${statusStyles[event.status] || "bg-gray-50 text-gray-700 border-gray-100"}`}
                      >
                  ...
                  <div className="p-5">
                    ...
                    {event.averageRating > 0 && (
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-sm font-semibold text-gray-800">{event.averageRating.toFixed(1)}</span>
                    ...
                    <div className="space-y-3 mb-4 text-sm text-gray-700">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-semibold text-violet-600">{remaining}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-2">
                      <Link
                        to={`/events/${event._id}`}
                        className="w-full text-center px-4 py-2.5 bg-white border border-violet-100 text-violet-700 font-semibold rounded-lg hover:bg-violet-50 transition"
                      >
                        View Details
                      </Link>
                      ...
                      <div className="flex gap-2">
                        <Link
                          ...
                          className="flex-1 text-center px-3 py-2 bg-violet-50 text-violet-700 font-medium rounded-lg hover:bg-violet-100 transition border border-violet-100"
                        >
                          Edit
                        </Link>
                        <button
